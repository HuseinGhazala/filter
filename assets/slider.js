...
function markInitialVisibleSlides(
  ...
  if (visible.length === 0) {
    ...
    } else if (currentBp && swiper.params?.breakpoints?.[currentBp]?.slidesPerView) {
      const bpSpv = swiper.params.breakpoints[currentBp].slidesPerView;
      if (typeof bpSpv === "number") {
        inViewCount = Math.max(1, bpSpv);
      }
    } else {
      const w = typeof window !== "undefined" ? window.innerWidth : 1200;
      if (w < 600) inViewCount = 2;
      else if (w < 1000) inViewCount = 3;
      else inViewCount = 4;
    }

    inViewCount = Math.min(swiper.slides.length, Math.max(1, inViewCount));
